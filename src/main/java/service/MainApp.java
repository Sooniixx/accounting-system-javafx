...
public class MainApp extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        empTabContent.setCenter(table);
        empTabContent.setLeft(createFormBox());
        empTabContent.setTop(createSearchBox());
        ...
        deptName.setPromptText("Назва відділу");
        ...
        addDeptBtn.setOnAction(e -> {
            ...
            deptList.getItems()
                    .setAll(controller.getAllDepartments().stream().map(d -> d.getId() + ": " + d.getName()).toList());
        });

        deptBox.getChildren().addAll(new Label("Назва:"), deptName, addDeptBtn, new Label("Список відділів:"),
                deptList);
    ...
    private void setupTable() {
        ...
        TableColumn<Employee, LocalDate> dateCol = new TableColumn<>("Дата прийому");
        dateCol.setCellValueFactory(new PropertyValueFactory<>("hireDate"));
        dateCol.setPrefWidth(120);
        TableColumn<Employee, Integer> annualCol = new TableColumn<>("Дні відпустки");
        annualCol.setCellValueFactory(new PropertyValueFactory<>("annualPaidLeaveDays"));
        annualCol.setPrefWidth(100);
        TableColumn<Employee, Integer> deptCol = new TableColumn<>("ID Відділу");
        deptCol.setCellValueFactory(new PropertyValueFactory<>("departmentId"));
        deptCol.setPrefWidth(80);
        TableColumn<Employee, Integer> carryCol = new TableColumn<>("Перенесені дні");
        carryCol.setCellValueFactory(new PropertyValueFactory<>("carryoverPaidLeaveDays"));
        carryCol.setPrefWidth(120);
        ...
        table.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                ...
                annualDaysField.setText(String.valueOf(newSelection.getAnnualPaidLeaveDays()));
                deptIdField.setText(String.valueOf(newSelection.getDepartmentId()));
                carryoverDaysField.setText(String.valueOf(newSelection.getCarryoverPaidLeaveDays()));
            }
        });
    }
    ...
    private VBox createFormBox() {
        ...
        annualDaysField = new TextField();
        annualDaysField.setPromptText("Днів щорічної відпустки");
        deptIdField = new TextField();
        deptIdField.setPromptText("ID Відділу (напр. 1)");
        carryoverDaysField = new TextField();
        carryoverDaysField.setPromptText("Перенесені дні");
        ...
        Button vacationButton = new Button("Оформити відпустку");
        vacationButton.setMaxWidth(Double.MAX_VALUE);
        vacationButton.setStyle("-fx-base: #ffcc66; -fx-font-weight: bold;");
        vacationButton.setOnAction(e -> openVacationDialog());
        Button analyticsButton = new Button("Аналітика: Залишок відпустки");
        analyticsButton.setMaxWidth(Double.MAX_VALUE);
        analyticsButton.setStyle("-fx-base: #66b3ff; -fx-font-weight: bold;");
        analyticsButton.setOnAction(e -> checkUnusedLeave());
        Button clearButton = new Button("Очистити форму");
        clearButton.setMaxWidth(Double.MAX_VALUE);
        clearButton.setOnAction(e -> clearForm());

        box.getChildren().addAll(titleLabel, nameField, positionField, hireDatePicker, annualDaysField, deptIdField,
                carryoverDaysField,
                new Separator(), addButton, updateButton, deleteButton, new Separator(), vacationButton,
                analyticsButton, new Separator(), clearButton);
        return box;
    }
    ...
    private HBox createSearchBox() {
        ...
        searchDeptField.setPromptText("ID відділу");
        ...
        searchPosField.setPromptText("Посада");
        Button searchPosBtn = new Button("Знайти");
        searchPosBtn.setOnAction(e -> searchByPosition(searchPosField.getText()));
        Button loadAllBtn = new Button("Показати всіх");
        loadAllBtn.setOnAction(e -> refreshTable());
        box.getChildren().addAll(new Label("Пошук за ID відділу:"), searchDeptField, searchDeptBtn, new Separator(),
                new Label("За посадою:"), searchPosField, searchPosBtn, new Separator(), loadAllBtn);
        return box;
    }
    ...
    private void openVacationDialog() {
        ...
        if (selected == null) {
            showError("Помилка", "Оберіть співробітника у таблиці!");
            return;
        }

        Stage dialog = new Stage();
        dialog.initModality(Modality.APPLICATION_MODAL);
        dialog.setTitle("Оформлення відпустки");
        VBox dialogVbox = new VBox(10);
        dialogVbox.setPadding(new Insets(20));

        DatePicker startDate = new DatePicker();
        startDate.setPromptText("Початок");
        DatePicker endDate = new DatePicker();
        endDate.setPromptText("Кінець");
        ...
        dialogVbox.getChildren().addAll(new Label("Співробітник: " + selected.getFullName()), new Label("Початок:"),
                startDate, new Label("Кінець:"), endDate, new Label("Тип:"), typeBox, saveBtn);
    ...
    private void addEmployee() {
        if (controller == null) {
            showError("Помилка", "Контролер не ініціалізовано!");
            return;
        }
        try {
    ...
    private void updateEmployee() {
        ...
        if (s != null) {
            try {
                controller.updateEmployee(s.getId(), nameField.getText(), positionField.getText(),
                        hireDatePicker.getValue(), Integer.parseInt(annualDaysField.getText()),
                        Integer.parseInt(deptIdField.getText()), Integer.parseInt(carryoverDaysField.getText()));
                refreshTable();
            } catch (Exception ex) {
                showError("Помилка", ex.getMessage());
            }
    ...
    private void deleteEmployee() {
        Employee s = table.getSelectionModel().getSelectedItem();
        if (s != null) {
            controller.deleteEmployee(s.getId());
            refreshTable();
            clearForm();
        }
    }
    ...
    private void checkUnusedLeave() {
        Employee selected = table.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showError("Помилка", "Оберіть співробітника!");
            return;
        }
        try {
            int remaining = controller.getUnusedLeaveDays(selected.getId());
            showInfo("Аналітика відпусток",
                    "Співробітник: " + selected.getFullName() + "\nДоступно днів оплачуваної відпустки: " + remaining);
        } catch (Exception ex) {
            showError("Помилка", ex.getMessage());
        }
    }

    private void searchByDepartment(String deptIdStr) {
        try {
            table.setItems(
                    FXCollections.observableArrayList(controller.searchByDepartmentId(Integer.parseInt(deptIdStr))));
        } catch (Exception ex) {
            showError("Помилка", "ID відділу має бути числом.");
        }
    }

    private void searchByPosition(String pos) {
        try {
            table.setItems(FXCollections.observableArrayList(controller.searchByPosition(pos)));
        } catch (Exception ex) {
            showError("Помилка", ex.getMessage());
        }
    }
    ...
    private void clearForm() {
        ...
        annualDaysField.clear();
        deptIdField.clear();
        carryoverDaysField.clear();
